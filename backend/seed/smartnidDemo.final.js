...
function statusHistory({status,createdAt,citizenId,reviewerId,reason}){
  ...
  if(['approved','printed','delivered'].includes(status)) s.push({ fromStatus:'under_review', toStatus:'approved', note:'Application approved. Biometric appointment is required.', changedAt:approveAt, changedBy:reviewerId, changedByRole:'admin', ipAddress:CFG.ipPool[1], userAgent:CFG.userAgent, requestId:`approve-${citizenId}` });
